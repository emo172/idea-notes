...
| `src/main/window/`                                                                        | `npm test -- tests/main/window-config.test.ts tests/main/window-bounds-persistence.test.ts tests/main/notification-window-opener.test.ts` |
...
| 桌面窄窗口、弹层遮挡、真实 UI 视觉风险                                                    | `npm test -- tests/renderer`，并手动检查 720px 主列表、概览、设置页、编辑器和确认弹窗                                                     |
...
UI 或样式改动至少手动检查约 720px 宽 Electron 主窗口下的主列表、概览、设置页、编辑器和确认弹窗。记录是否存在明显水平溢出、文字重叠、按钮挤压或弹层遮挡；如本地 Electron 环境阻塞，需在 PR 中写明命令输出和阻塞原因。

## PR 要求

- PR 必须来自非 `main` 分支；不要用本地 `main` 直接承载待合并改动。
- PR 描述必须说明改动范围、风险和验证命令。
- UI 或样式改动需要提供截图或录屏，并写明截图/验证覆盖范围；若无法截图，必须说明未截图原因、已运行的 CSS 契约测试和 720px 手动检查结果。
...
`npm run smoke` 只检查生产构建产物，不启动真实 Electron 窗口。真实桌面截图自动化是后续独立工作；若引入 Playwright、Xvfb 或其它工具链，需要单独更新 `package.json`、CI、README 和本文件，本阶段不通过新增依赖解决。
