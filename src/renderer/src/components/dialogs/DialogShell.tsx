...
export function DialogShell({
  ...
  const { panelRef, handleKeyDown } = useFocusTrap(onEscape);
  const overlayClassNames = ["dialog-overlay", overlayClassName]
    .filter(Boolean)
    .join(" ");
  const panelClassNames = ["dialog-panel", panelClassName].filter(Boolean).join(" ");
  const headerClassNames = ["dialog-head", headerClassName].filter(Boolean).join(" ");
  const bodyClassNames = ["dialog-body", bodyClassName].filter(Boolean).join(" ");
