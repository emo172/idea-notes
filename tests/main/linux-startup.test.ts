...
describe("Linux 开发启动约定", () => {
  ...
  it("为 Linux 开发窗口声明稳定桌面标识", () => {
    ...
    expect(beforeAppReady).toContain("configureLinuxDevelopmentStartup(app)");
  ...
  it("在应用 ready 前禁用 Linux 开发环境 GPU sandbox 且不使用矛盾 GPU 开关", () => {
    ...
    expect(platformSource).not.toContain('appendSwitch("disable-gpu")');
